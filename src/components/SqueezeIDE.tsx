          ...
          {activeTab === 'map' && (
            <div className="flex-1 p-4 overflow-hidden">
               <InteractiveGameMap 
                 ...
                 onSendPromptToAgent={(prompt) => {
                   setPendingAgentPrompt(prompt);
                   setActiveTab('chats');
                 }}
                 isStudioConnected={!!isConnected}
               />
            </div>
          )}

          {activeTab === 'settings' && (
            <div className="flex-1 p-8 text-sm max-w-2xl overflow-y-auto">
              <h2 className="text-xl font-bold mb-2 text-white">IDE & Model Settings</h2>
              <p className="text-white/50 mb-6">Configure AI models, Bring Your Own Key (BYOK), execution parameters, and WebSync.</p>
              
              <div className="space-y-6">
                {/* AI Model Connection & BYOK Panel */}
                <AISettingsPanel 
                  onShowToast={onShowToast}
                />
