...
export function buildDynamicGameMap(files: ProjectFileInfo[], projectName: string = "Roblox Game"): ProjectHealthAudit {
  ...
  const activeSubsystems = discovered.filter(s => s.files.length > 0);
  const totalActive = activeSubsystems.length;
  const radius = 320;

  activeSubsystems.forEach((sys, idx) => {
    const angle = (idx / totalActive) * 2 * Math.PI - Math.PI / 2;
    ...
    nodes.push({
      ...
      description: `${sys.description} (${sys.files.length} script${sys.files.length === 1 ? '' : 's'})`,
