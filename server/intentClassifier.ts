...
export function classifyUserIntent(prompt: string, contextFiles?: ProjectFileInfo[]): DetectedIntentResult {
  ...
  // 1. Greetings (Arabic + English)
  if (/^(hi|hey|hello|yo|sup|greetings|howdy|what's up|whats up|good morning|good evening|good afternoon|who are you|what can you do|help me|what are you|مرحبا|أهلا|اهلا|سلام|السلام عليكم|هاي|مين انت|ايش تقدر تسوي|من انت|هلا)(\s|!|\.|\?|$)/i.test(p)) {
    return {
      intent: 'GREETING',
      confidence: 0.99,
  ...
  if (/^(sync to studio|push to studio|check studio|studio status|get explorer|connect studio|websync status|اربط الاستديو|مزامنة|مزامنه|ارسل للاستديو|تحديث الاستديو)/i.test(p) || p === 'sync') {
    return {
      intent: 'STUDIO_SYNC',
      confidence: 0.98,
  ...
  // 3. Project-wide Read / Audit Requests (Arabic + English)
  const isArabicProjectQuery = /(اقرأ|اقرا|افحص|حلل|شوف|راجع|اشرح|فحص|تحليل|شرح)\s+(مشروعي|المشروع|اللعبة|اللعبه|الكود|الملفات|السيستم|الأنظمة|الانظمة)/i.test(p) ||
    /اللعبة دي عبارة عن إيه|اللعبه دي بتعمل ايه|اللعبة عبارة عن ايه|اللعبه عن ايه|اشرح اللعبه|اشرح اللعبة|اقرا اللعبه|اقرأ اللعبة|شو هي اللعبة|ايش فكرة اللعبة/i.test(p);

  if (/^(read my project|analyze my project|analyze codebase|audit my code|inspect project|review my code|what does my game do|summarize my game|project overview|game structure|audit project|what systems does my game have|explain my game|tell me about my game)/i.test(p) ||
      p.includes('read my project') || p.includes('analyze my project') || p.includes('audit my codebase') || p.includes('explain my game') || isArabicProjectQuery) {
    return {
  ...
  const isArabicExplainQuery = /(اشرح هذا|اشرح الكود|ايش يسوي هذا الكود|الكود ده بيعمل ايه|اشرح السكربت|ما وظيفة هذا الكود|فهميني الكود)/i.test(p);
  ...
  // 5. DEBUG / ERROR DIAGNOSIS (Arabic + English)
  const isArabicDebugQuery = /(صلح|عدل الخطأ|فيه مشكلة|مش شغال|ما يشتغل|في مشكلة|debug|error|مشكله|خطأ|خطا|صلحه|ليش مو شغال|ليه مش شغال|الـQuest مش شغال|السكربت خربان|عندي ايرور|فيه ايرور)/i.test(p);
  const isDebugQuery = /^(there is a bug|there's a bug|why am i getting this error|why does this (error|fail|crash|break)|why is this error happening|what is causing this (error|bug|issue)|why isn't this working|why is this nil|debug this|diagnose this|fix this|fix error)/i.test(p) ||
    p.includes('there is a bug') || p.includes('there\'s a bug') || p.includes('why am i getting this error') || p.includes('attempt to index nil') || isArabicDebugQuery;
  ...
  // 8. FEATURE BUILD & MODIFICATION (Multi-file / Multi-component gameplay system request in Arabic + English)
  const isArabicBuild = /(اعمل|سوي|سو|انشئ|أنشئ|برمج|ضيف|أضف|ابني|ابن|ركب|حط|طور|جهز)\s+(نظام|سيستم|سكربت|ميزة|خاصية|موديل|بوثات|كويست|متجر|ليدربورد|داتا|حفظ|انفنتوري|تداول|كومبات|طيران|دبل جمب|جوائز|rewards|quest|booth|shop|inventory|combat|trading|leaderboard|datastore|daily|system|mechanic|gui|ui)/i.test(p) ||
    /(خليه أسرع|خليه اسرع|غير النظام|عدل اللي عملناه|اعملها زي|اعمل كل حاجة|ضيف نظام)/i.test(p);
  const isBuildFeature = /(^(make|create|write|build|code|implement|generate|develop|add a system|add a mechanic|set up|modify|upgrade|speed up)\b)|(build (a|an|the)|create (a|an|the)|make (a|an|the)|add (a|an|the))/i.test(p) || isArabicBuild;
  
  if (isBuildFeature) {
    return {
      intent: 'FEATURE_BUILD',
      confidence: 0.95,
      reason: 'User explicitly requested building or modifying a complete game feature or mechanics system.',
      hasCodeInPrompt,
      requiresCodeGeneration: true,
      mode: 'FEATURE_MODE',
