...
Return your analysis in the specified JSON format.`;
    ...
    let parsed: any = {};
    try {
      parsed = await executeWithModelFallback(
        ai,
        'memory',
        prompt,
        "You are an expert AI Memory Extraction agent.",
        { responseSchema: schemaConfig.responseSchema, temperature: 0.1 }
      );
    } catch (err: any) {
      console.warn("[Memory Service] AI memory extraction error, falling back to legacy/heuristic:", err.message || err);
      parsed = {};
    }
