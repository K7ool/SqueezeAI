...
export const studio = {
  ...
  scan_project: async (projectId: string) => {
    const syncState = studioWebSync.getProjectSyncState(projectId);
    const tree = studioWebSync.getMemoryTree(projectId);
    return {
      success: true,
  ...
  readOutput: async (projectId: string, limit: number = 50) => {
    const logs = db.getStudioAuditLogs(projectId, limit);
    return {
      success: true,
      logs: logs.filter(l => l.type === 'STUDIO_INFO' || l.type === 'STUDIO_WARN' || l.type === 'STUDIO_ERROR' || l.type === 'AI_PUBLISH')
    };
  },
  read_output: async (projectId: string, limit: number = 50) => {
    const logs = db.getStudioAuditLogs(projectId, limit);
    return {
      success: true,
      logs: logs.filter(l => l.type === 'STUDIO_INFO' || l.type === 'STUDIO_WARN' || l.type === 'STUDIO_ERROR' || l.type === 'AI_PUBLISH')
  ...
  getErrors: async (projectId: string, limit: number = 10) => {
    const errors = db.getRecentErrors('usr_demo_builder', projectId, limit);
    return {
      success: true,
      errors
    };
  },
  get_errors: async (projectId: string, limit: number = 10) => {
    const errors = db.getRecentErrors('usr_demo_builder', projectId, limit);
    return {
      success: true,
      errors
