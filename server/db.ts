...
export interface UserMemoryRecord {
  id: string;
  userId: string;
  type: 'preference' | 'coding_style' | 'luau_style' | 'architecture' | 'response_style' | 'tool_preference' | 'explicit_statement' | 'explicit_configuration';
  key: string;
  value: any;
...
export interface AiSettingsRecord {
  userId: string;
  provider: 'gemini' | 'openrouter' | 'opencode_zen';
  model: string;
...
async function syncUpsert(table: keyof DatabaseSchema, record: any) {
  try {
    await queueSupabaseWrite(async () => {
      ...
      console.log(`[Supabase Write Start] Table: '${mappedTable}' | PK: '${pk}'=${cleanRecord[pk]}`);
      ...
      if (error && error.message && error.message.includes('violates foreign key constraint') && cleanRecord.userId) {
        console.warn(`[Supabase FK Fix] Auto-upserting parent user '${cleanRecord.userId}'...`);
        const data = ensureDb();
      ...
      // If table is missing from schema cache (PGRST205), fallback gracefully
      if (error && error.code === 'PGRST205') {
        if (table === 'projects' || mappedTable === 'projects') {
          console.warn(`[Supabase Fallback] Table 'projects' missing in DB. Fallback syncing to 'project_memories'...`);
          const fallbackRecord = {
            id: cleanRecord.id || ('prj_mem_' + cleanRecord.projectId),
            userId: cleanRecord.userId,
            projectId: cleanRecord.id || cleanRecord.projectId || 'prj_default',
            projectName: cleanRecord.name || cleanRecord.projectName || 'Roblox Place',
            createdAt: cleanRecord.createdAt || new Date().toISOString(),
            updatedAt: cleanRecord.updatedAt || new Date().toISOString(),
          };
          await supabase.from('project_memories').upsert([fallbackRecord], { onConflict: 'id' });
          return;
        }
        if (table === 'aiSettings' || mappedTable === 'ai_settings') {
          console.warn(`[Supabase Fallback] Table 'ai_settings' missing in DB. Fallback syncing to 'user_memories'...`);
          const fallbackRecord = {
            id: 'mem_usr_aisettings_' + cleanRecord.userId,
            userId: cleanRecord.userId,
          ...
          await supabase.from('user_memories').upsert([fallbackRecord], { onConflict: 'id' });
          return;
        }
      }

      if (error) {
        console.error(`❌ [Supabase Write FAILED] Table: '${mappedTable}' | Code: ${error.code} | Msg: ${error.message}`);
      } else {
        console.log(`✅ [Supabase Write SUCCESS] Table: '${mappedTable}' | Record ID: ${cleanRecord[pk]}`);
      }
    });
  } catch (err: any) {
    console.error(`❌ [Supabase Write Exception] ${table}:`, err?.message || err);
  }
}
...
export let supabaseSyncErrors: Array<{ table: string; code: string; message: string }> = [];
export let hasAttemptedSupabaseSync = false;
  ...
  initPromise = (async () => {
    ...
    try {
      ...
      const promises = tables.map(async (table) => {
        ...
        if (error) {
          console.error(`❌ [Supabase Init FAILED] Table: '${table}' | Code: ${error.code} | Msg: ${error.message}`);
          supabaseSyncErrors.push({ table, code: error.code, message: error.message });
      ...
      for (const table of BULK_SYNC_ORDER) {
        const recordsToUpsertRaw = data[table] as any[];
        let recordsToUpsert = recordsToUpsertRaw;
        
        if (table === 'executionMemories') {
          const conversations = data['conversations'] as any[];
          const conversationIds = new Set(conversations.map(c => c.id));
          recordsToUpsert = recordsToUpsertRaw.filter(r => conversationIds.has(r.conversationId));
          
          if (recordsToUpsert.length < recordsToUpsertRaw.length) {
            console.warn(`⚠️ Filtering out ${recordsToUpsertRaw.length - recordsToUpsert.length} orphaned execution memories.`);
          }
        }
        ...
        if (recordsToUpsert && recordsToUpsert.length > 0) {
          ...
          if (mappedTable && pk) {
            ...
            if (upsertError) {
              console.error(`❌ [Supabase Sync-Back FAILED] Table: '${table}' | Code: ${upsertError.code} | Msg: ${upsertError.message}`);
              if (!supabaseSyncErrors.some(e => e.table === table)) {
                supabaseSyncErrors.push({ table, code: upsertError.code, message: upsertError.message });
              }
            }
          }
        }
      ...
      if (supabaseSyncErrors.length === 0) {
        console.log('🎉 Squeeze Cache successfully synchronized with Supabase.');
      } else {
        console.warn(`⚠️ Squeeze Cache partially synchronized. ${supabaseSyncErrors.length} tables had errors (possibly missing schema).`);
      }
    } catch (err: any) {
      ...
      supabaseSyncErrors.push({ table: 'all', code: 'UNREACHABLE', message: err.message || 'Connection timeout or network error' });
      hasLoadedFromSupabase = true;
    } finally {
      initPromise = null;
    }
...
part.Touched:Connect(onTouch)`,
    ...
    projects: [
      {
        id: 'prj_default',
        userId: 'usr_demo_builder',
        name: 'Squeeze Main Roblox Place',
        description: 'Primary active Roblox Studio workspace project',
        placeId: 12345678,
        universeId: 87654321,
  ...
  try {
    ...
    memoryDb = {
      ...
      aiSettings: parsed.aiSettings || initialDb.aiSettings,
      ...
      userMemories: (() => {
        const uniqueUserMemoriesMap = new Map<string, any>();
        const rawUserMemories = parsed.userMemories || [];
        for (const m of rawUserMemories) {
          if (!m.key) continue;
          const keyCombo = `${m.userId || 'usr_demo_builder'}-${m.key}`;
          const existing = uniqueUserMemoriesMap.get(keyCombo);
          if (!existing) {
            uniqueUserMemoriesMap.set(keyCombo, m);
          } else {
            const existingTime = new Date(existing.updatedAt || existing.createdAt || 0).getTime();
            const currentTime = new Date(m.updatedAt || m.createdAt || 0).getTime();
            if (currentTime > existingTime) {
              uniqueUserMemoriesMap.set(keyCombo, m);
            }
          }
        }
        return Array.from(uniqueUserMemoriesMap.values());
      })(),
      ...
      conversationStates: parsed.conversationStates || []
    };
    return memoryDb;
  } catch (err) {
...
function saveDb(data: DatabaseSchema) {
  ...
  try {
    if (process.env.NODE_ENV === 'production') {
      ...
      // We rely solely on Supabase persistence in production.
      return;
    }
    if (!fs.existsSync(DATA_DIR)) {
      fs.mkdirSync(DATA_DIR, { recursive: true });
...
export const db = {
  ...
  createUser(user: Omit<UserRecord, 'id' | 'createdAt' | 'updatedAt' | 'usedGenerations' | 'quotaResetDate' | 'passwordHash' | 'plan' | 'planStatus' | 'monthlyLimit'> & {
    id?: string;
    passwordHash?: string;
    plan?: 'free' | 'pro' | 'studio';
    planStatus?: 'active' | 'past_due' | 'canceled';
  ...
  getConversations(userId: string, projectId?: string, search?: string): ConversationRecord[] {
    const data = ensureDb();
    console.log(`[DB Query] getConversations for userId='${userId}' projectId='${projectId || 'all'}'`);
    let list = data.conversations.filter(c => (c.userId === userId || userId === 'usr_demo_builder' || c.userId === 'usr_demo_builder') && !c.archived);
    if (projectId) {
      const projectMatches = list.filter(c => c.projectId === projectId);
    ...
    const sorted = list.sort((a, b) => new Date(b.lastMessageAt || b.createdAt).getTime() - new Date(a.lastMessageAt || a.createdAt).getTime());
    console.log(`[DB Query Result] getConversations found ${sorted.length} conversations`);
    return sorted;
  },
  ...
  getConversationById(id: string, userId?: string): ConversationRecord | undefined {
    ...
    if (!conv) {
      console.log(`[DB Query] getConversationById '${id}' NOT found`);
      return undefined;
    }
    if (userId && conv.userId !== userId && userId !== 'usr_demo_builder' && conv.userId !== 'usr_demo_builder') {
      console.warn(`[DB Query] getConversationById '${id}' user mismatch (${conv.userId} vs ${userId})`);
      return undefined;
    }
    return conv;
  },
  ...
  createConversation(entry: Partial<ConversationRecord> & Omit<ConversationRecord, 'id' | 'createdAt' | 'updatedAt' | 'lastMessageAt' | 'messageCount' | 'archived'> & { id?: string }): ConversationRecord {
    const data = ensureDb();
    const now = new Date().toISOString();
    const convId = entry.id || ('conv_' + crypto.randomUUID().slice(0, 8));
    
    // Check if conversation with this ID already exists
    const existingIdx = data.conversations.findIndex(c => c.id === convId);
    if (existingIdx !== -1) {
      console.log(`[DB] Conversation '${convId}' already exists, returning existing.`);
      return data.conversations[existingIdx];
    }

    const conv: ConversationRecord = {
      userId: entry.userId || 'usr_demo_builder',
      projectId: entry.projectId || 'prj_default_roblox',
      title: entry.title || 'New Roblox System',
      id: convId,
      createdAt: now,
      updatedAt: now,
      lastMessageAt: now,
    ...
    saveDb(data);
    console.log(`[DB] Created conversation in DB: id='${conv.id}' userId='${conv.userId}' title='${conv.title}'`);
  ...
  deleteConversation(id: string, userId: string): boolean {
    ...
    data.conversations = data.conversations.filter(c => !(c.id === id && (c.userId === userId || userId === 'usr_demo_builder' || c.userId === 'usr_demo_builder')));
  ...
  getMessages(conversationId: string, limit: number = 100, offset: number = 0): ChatMessageRecord[] {
    ...
    console.log(`[DB Query] getMessages for conversationId='${conversationId}' returned ${msgs.length} messages.`);
    return msgs.slice(offset, offset + limit);
  },

  createMessage(msg: Omit<ChatMessageRecord, 'id' | 'timestamp'> & { id?: string }): ChatMessageRecord {
    const data = ensureDb();
    const now = new Date().toISOString();
    const newMsg: ChatMessageRecord = {
      ...msg,
      id: msg.id || ('msg_' + crypto.randomUUID().slice(0, 8)),
      timestamp: now,
    };
    ...
    if (conv) {
      ...
      conv.updatedAt = now;
      if ((!conv.title || conv.title === 'New Chat' || conv.title === 'New Roblox System') && msg.role === 'user') {
        conv.title = msg.content.slice(0, 45).trim() || 'New Chat';
      }
    }
    saveDb(data);
    console.log(`[DB] Created message in DB: id='${newMsg.id}' convId='${newMsg.conversationId}' role='${newMsg.role}' text='${newMsg.content.slice(0, 35)}...'`);
  ...
  getUserMemories(userId: string): UserMemoryRecord[] {
    const data = ensureDb();
    const rawList = data.userMemories.filter(m => m.userId === userId || userId === 'usr_demo_builder' || m.userId === 'usr_demo_builder');
    
    // Deduplicate by memory key, preserving the most recently updated entry
    const map = new Map<string, UserMemoryRecord>();
    for (const m of rawList) {
      ...
      if (!existing) {
        ...
        const existingTime = new Date(existing.updatedAt || existing.createdAt || 0).getTime();
        const currentTime = new Date(m.updatedAt || m.createdAt || 0).getTime();
        if (m.userId === userId || currentTime > existingTime) {
          map.set(m.key, m);
        }
      }
    }
  ...
  getUserMemoryByKey(userId: string, key: string): UserMemoryRecord | undefined {
    const data = ensureDb();
    const matches = data.userMemories.filter(m => (m.userId === userId || userId === 'usr_demo_builder' || m.userId === 'usr_demo_builder') && m.key === key);
  ...
  saveUserMemory(mem: Omit<UserMemoryRecord, 'id' | 'createdAt' | 'updatedAt' | 'version'>): UserMemoryRecord {
    ...
    // Match existing memory by key across user IDs
    const existingIdx = data.userMemories.findIndex(m => (m.userId === mem.userId || m.userId === 'usr_demo_builder' || mem.userId === 'usr_demo_builder') && m.key === mem.key);
    ...
    if (existingIdx >= 0) {
      ...
      // Clean up any other duplicate records with the same key
      data.userMemories = data.userMemories.filter((m, idx) => idx === existingIdx || m.key !== mem.key);
    } else {
      result = {
        ...mem,
